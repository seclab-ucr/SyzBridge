...
class SymExec(MemInstrument):
    ...
    def _init_regs(self):
        self._regs = ['rdi', 'rsi', 'rdx', 'r10', 'r8', 'r9']
        self._reg_offset = {'rdi': 0x70, 'rsi': 0x68, 'rdx': 0x60, 'r10': 0x38, 'r8': 0x48, 'r9': 0x40}
    ...
    def run_sym(self, entry_func, path=[], terminating_func='', raw_tracing=False, timeout=60*10):
        dfs = True
        ...
        if not self._context_ready:
            ...
            self.vm.lock_thread()
            rdi_value = self._read_rdi()
            if rdi_value == None:
                self.logger.error("Cannot locate the entry function")
                raise VulnerabilityNotTrigger
            self._ori_rdi = rdi_value
    ...
    def _symbolize_vuln_mem(self):
        n = self._number_of_arguments(self.entry_func)
        for reg in self._regs[:n]:
            self.make_symbolic(self._init_state, self._ori_rdi + self._reg_offset[reg], 0x4, "s_obj_{}".format(reg))
